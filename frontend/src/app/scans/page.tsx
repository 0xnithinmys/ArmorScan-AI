          ...
          <form onSubmit={createScan} className="h-fit rounded-2xl border border-white/7 bg-[#080f18] p-5">
            ...
            <div className="mt-5 space-y-3">
              ...
              {form.target_id === "__new__" && (
                <>
                  <input className="field" placeholder="Inline target name (optional)" value={form.target_name} onChange={e => setForm({ ...form, target_name: e.target.value })} />
              ...
              {streamEvents.length === 0 ? <EmptyState text="Select a scan to watch worker events from the backend WebSocket stream." /> : (
                <div className="space-y-2">
                  {streamEvents.map((event, index) => (
                    <pre key={`${event}-${index}`} className="overflow-auto rounded-xl border border-white/7 bg-[#05090f] p-3 font-mono text-[10px] leading-5 text-white/55">
                      {event}
                    </pre>
                  ))}
                </div>
